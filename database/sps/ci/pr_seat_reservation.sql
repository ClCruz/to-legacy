CREATE PROCEDURE dbo.pr_seat_reservation (@id_apresentacao INT, @indice INT, @id VARCHAR(100), @NIN VARCHAR(10), @minutesToExpire INT, @codCliente INT = NULL, @codReserva VARCHAR(10) = NULL)
...
INNER JOIN CI_MIDDLEWAY..mw_apresentacao ap ON a.CodApresentacao=ap.CodApresentacao
WHERE 
    ap.id_apresentacao=@id_apresentacao AND ls.Indice=@indice AND ls.StaCadeira='R'-- AND ls.id_session!=@id_session
