...
if (acessoPermitido($mainConnection, $_SESSION['admin'], 370, true)) {
    ...
    if (isset($_GET['action'])) {
        ...
        $query = "SELECT 
                    ...
                    WHERE (C.CD_CPF = ? OR ? = '') AND PR.ID_PACOTE = ? AND PR.IN_ANO_TEMPORADA = ?
                    AND IN_STATUS_RESERVA IN ('A', 'S')
                    ORDER BY E.DS_EVENTO, TS.NOMSETOR, PR.DS_LOCALIZACAO";
        $params = array($_POST['cpf'], $_POST['cpf'], $_POST['pacote_combo'], $_POST['ano']);
...
<h2>Liberação de Lugar para Venda</h2>
<form id="dados" name="dados" method="post">
    <table class="tb-form">
        ...
        <tr>
            ...
            <td>
                <input type="text" id="ano"  name="ano" value="<?php echo $_POST['ano']; ?>" />
            </td>
        </tr>
        <tr>
            <td class="coluna-header"><strong>CPF do Assinante (apenas números)</strong></td>
            <td>
                <input type="text" id="cpf" name="cpf" value="<?php echo $_POST['cpf']; ?>" />
            </td>
            <td class="coluna-header"><strong>Nome do Assinante</strong></td>
            <td>
                <?php echo $rsAux['DS_NOME_SOBRENOME']; ?>
            </td>
        </tr>    
        <tr>
            <td></td>
